...
begin
  ...
  Writeln('=============================================================================');
  WriteLn( Format(#10'Lock plus unlock on an unlocked mutex.'#10'%d iterations'#10#10,
          [ITERATIONS]));
  ...
  WriteLn( Format('%-45s %15d %15.3f'#10,
      ['Simple Critical Section',
          durationMilliSecs, ( durationMilliSecs * 1E3 / ITERATIONS / 4 )]
          ));
  ...
  running := 0;
  if old_mutex_unlock(@ox1) <> 0then
    exit;
  if old_mutex_unlock(@ox2) <> 0then
    exit;
  if pthread_join(worker, nil) <> 0then
    exit;
  if old_mutex_destroy(@ox2) <> 0then
    exit;
  if old_mutex_destroy(@ox1) <> 0then
    exit;
  stoptime := GetTickCount;
  durationMilliSecs := stoptime - starttime;
  WriteLn( Format('%-45s %15d %15.3f'#10,
